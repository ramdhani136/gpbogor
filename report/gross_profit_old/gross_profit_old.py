	...
	def load_invoice_items(self):
		...
		self.si_list = frappe.db.sql("""
			select
				...
				dni.warehouse, `tabSales Invoice Item`.item_group,
			...
			from
				...
				ON dni.`parent` = `tabSales Invoice Item`.delivery_note
				AND dni.`item_code` = `tabSales Invoice Item`.`item_code`
				 AND dni.docstatus = 1
			where
			...
			.format(conditions=conditions, sales_person_cols=sales_person_cols,
				sales_team_table=sales_team_table, match_cond = get_match_cond('Sales Invoice')), self.filters, as_dict=1,debug=1)
